package com.optic.deliveryapp.activities.client.address.create
...
import com.optic.deliveryapp.activities.client.address.map.ClientAddressMapActivity
...
class ClientAddressCreateActivity : AppCompatActivity() {
    ...
    private fun isValidForm(address: String, neighborhood: String): Boolean{
        if (address.isNullOrBlank()){
            Toast.makeText(this, "Ingresa la dirección", Toast.LENGTH_LONG).show()
            return false
        }
        if (neighborhood.isNullOrBlank()){
            Toast.makeText(this, "Ingresa el vecindario", Toast.LENGTH_LONG).show()
            return false
        }
        if (addressLat == 0.0){
            Toast.makeText(this, "Selecciona el punto de referencia", Toast.LENGTH_LONG).show()
            return false
        }
        if (addressLng == 0.0){
            Toast.makeText(this, "Selecciona el punto de referencia", Toast.LENGTH_LONG).show()
